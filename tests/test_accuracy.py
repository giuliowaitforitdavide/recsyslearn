...
class NDCGTest(unittest.TestCase):
    ...
    def test_ndcg(self) -> None:
        ...
        ndcg_vals = pd.DataFrame(
            [
                ['1', (1 / np.log(1 + 1) + 0 / np.log(2 + 1)) / (1 / np.log(1 + 1) + 1 / np.log(2 + 1))],
                ['2', (0 / np.log(1 + 1) + 0 / np.log(2 + 1)) / (1 / np.log(1 + 1) + 1 / np.log(2 + 1))],
                ...
                ['6', (1 / np.log(1 + 1) + 0 / np.log(2 + 1)) / (1 / np.log(1 + 1) + 0 / np.log(2 + 1))]
            ], columns = ['user', 'NDCG@2']
        )
    ...
    def test_ndcg_error(self) -> None:
        ...
        self.assertTrue("Values for the recommendation list length is not compatbile with the dataset. ats=(3,)" in str(context.exception))
