from recsyslearn.beyond_accuracy.metrics import *
...
coverage = Coverage().evaluate(top_k_with_item_groups,
                               segmented_items['item'].to_list())
novelty = Novelty().evaluate(top_k_with_item_groups)

print(
    json.dumps({'coverage': coverage, 'Novelty': novelty}, indent=4)
)
