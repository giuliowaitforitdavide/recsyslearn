...
class MutualInformation(FairnessMetric):
    ...
    def evaluate(self, top_n: pd.DataFrame, flag: str, rel_matrix: pd.DataFrame = None) -> float:
        ...
        P_xy = top_n[[not_grouped, 'group', 'rank']].groupby([not_grouped, 'group'], as_index=False).sum()
        P_xP_y = top_n[[not_grouped, 'group', 'rank']].groupby(not_grouped, as_index=False).sum()
        ...
        tmp = P_xP_y[[not_grouped, 'group', 'rank']].merge(P_xy, on=[not_grouped, 'group'])
