...
class Coverage(Metric):
  ...
  def evaluate(self, top_n: pd.DataFrame, items: list) -> float:
    ...
    ------
    Exception
      If top_n pd.DataFrame not in the form ('user', 'item', 'rank', 'group').
...
class Novelty(Metric):
  ...
  def evaluate(self, top_n: pd.DataFrame) -> float:
    ...
    ------
    Exception
      If top_n pd.DataFrame not in the form ('user', 'item', 'rank', 'group') or if item_group has values not in (0, 1, 2).
...
class Entropy(Metric):
  ...
  def evaluate(self, top_n: pd.DataFrame, rel_matrix: pd.DataFrame = None) -> float:
    ...
    ------
    Exception
      If top_n DataFrame not in the form ('user', 'item', 'rank', 'group').
...
class KullbackLeibler(Metric):
  ...
  def evaluate(self, top_n: pd.DataFrame, target_representation: pd.DataFrame, rel_matrix: pd.DataFrame = None) -> float:
    ...
    target_representation : DataFrame
    ...
    Exception
      If top_n DataFrame not in the form ('user', 'item', 'rank', 'group').
      If target_representation DataFrame not in the form ('group', 'target_representation').
