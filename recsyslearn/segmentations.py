...
from recsyslearn.errors import (
...
class InteractionSegmentation(Segmentation):
    ...
    def segment(
        ...
        proportion : list, default [0.8, 0.2]
                ...
                Its length should be between 1 and 3.
        ...
        WrongProportionsException
                If sum(proportion) is not 1,
                which means it doesn't cover all the items/users.
        ...
        mid_tail = tmp.loc[
            tmp["cumulative_sum"].lt(mid_thr) & ~tmp["item"].isin(short_head),
            "item",
        ]
        conditions = [
            item_groups["item"].isin(short_head),
            item_groups["item"].isin(mid_tail),
        ]
        choices = ["1", "2"]
        item_groups.loc[:, "group"] = np.select(
            conditions, choices, default="3" if len(proportion) == 3 else "2"
        )
...
class ActivitySegmentation(Segmentation):
    ...
    def segment(
        ...
        threshold = (
            round(user_groups.shape[0] * proportion[0])
            if round(user_groups.shape[0] * proportion[0]) > 0
            else 1
        )
